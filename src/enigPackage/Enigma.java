...
public class Enigma {
	...
	// A helper method for encryptor that recursively passes a character currChar through all the 
	//		rotors and reflector of a given Enigma and handles rotor position rotations for all 
	// 		3 main rotors.
	private char encryptorHelper(char currChar, RotorList currRotorList) {
		if (currRotorList.previous == null) {
			currRotorList.rotor.rotate();
				...
				currRotorList.previous.rotor.getJustRotated()) {
			currRotorList.rotor.rotate();
